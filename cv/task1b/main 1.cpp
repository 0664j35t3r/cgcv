...
void matchHistograms( Mat patch_src, Mat patch_dst, Mat &matched )
{
  ...
  Mat hist_src_a = getHistogram( channel_src[1] );
  Mat hist_src_b = getHistogram( channel_src[2] );
  Mat hist_dst_a = getHistogram( channel_dst[1] );
  Mat hist_dst_b = getHistogram( channel_dst[2] );
  
  normalize(hist_src_a, hist_src_a, NORM_L1);
  normalize(hist_src_b, hist_src_b, NORM_L1);
  normalize(hist_dst_a, hist_dst_a, NORM_L1);
  normalize(hist_dst_b, hist_dst_b, NORM_L1);
  ...
  float sum_src_cdf_a = 0;
  float sum_src_cdf_b = 0;
  float sum_dst_cdf_a = 0;
  float sum_dst_cdf_b = 0;

  for (int i = 0; i < 256; i++)
  {
    sum_src_cdf_a += hist_src_a.at<float>(0, i);
    sum_src_cdf_b += hist_src_b.at<float>(0, i);
    sum_dst_cdf_a += hist_dst_a.at<float>(0, i);
    sum_dst_cdf_b += hist_dst_b.at<float>(0, i);
    src_cdf_a.at<float>(i) = sum_src_cdf_a;
    src_cdf_b.at<float>(i) = sum_src_cdf_b;
    dst_cdf_a.at<float>(i) = sum_dst_cdf_a;
    dst_cdf_b.at<float>(i) = sum_dst_cdf_b;
  }

  Vec<uchar, 256> lut_scr_a;
  Vec<uchar, 256> lut_scr_b;
  ...
  for (int i = 0; i < 256; i++)
  {
    value = dst_cdf_a.at<float>(i);
    index = 0;
    diff_prev = value;
    for (int j = 0; j < 256; j++)
    {
      diff_now = abs(value - src_cdf_a.at<float>(j));
      if (((diff_prev == diff_now) && (abs(index - i) > abs(j - i))) ||
           (diff_prev > diff_now))
      {
        index = j;
        diff_prev = diff_now;
    ...
    lut_scr_a[i] = (uchar)index;
  }
  
  for (int i = 0; i < 256; i++)
  {
    value = dst_cdf_b.at<float>(i);
    index = 0;
    diff_prev = value;
    for (int j = 0; j < 256; j++)
    {
      diff_now = abs(value - src_cdf_b.at<float>(j));
      if (((diff_prev == diff_now) && abs(index - i) > abs(j - i)) ||
           (diff_prev > diff_now))
      {
        index = j;
  ...
  for (int y = 0; y < patch_src.rows; y++)
  {
    ...
    {
      channel_dst[1].at<uchar>(x, y) = lut_scr_a[channel_dst[1].at<uchar>(x, y)];
      channel_dst[2].at<uchar>(x, y) = lut_scr_b[channel_dst[2].at<uchar>(x, y)];
    }
  }
...
//  
//  normalize(src_hist_a, src_hist_a, NORM_L1);
//  normalize(src_hist_b, src_hist_b, NORM_L1);
//  normalize(dst_hist_a, dst_hist_a, NORM_L1);
//  normalize(dst_hist_b, dst_hist_b, NORM_L1);
//  
//  // calculate cdf
//  Mat src_cdf_a = Mat::zeros( 1, 256, CV_32FC1 );
//  Mat src_cdf_b = Mat::zeros( 1, 256, CV_32FC1 );
...
//  calculateLut(lut_scr_a, dst_cdf_a, src_cdf_a);
//  calculateLut(lut_scr_b, dst_cdf_b, src_cdf_b);
//  
